...
global.dfail = (type, m, conn) => {
  const msg = {
    ...
    admin:    '⚠️ 𝘕𝘦𝘤𝘦𝘴𝘪𝘵𝘢𝘴 𝘴𝘦𝘳 𝘢𝘥𝘮𝘪𝘯 𝘚𝘰𝘭𝘰 𝘭𝘰𝘴 𝘢𝘥𝘮𝘪𝘯𝘴 𝘱𝘶𝘦𝘥𝘦𝘯 𝘶𝘴𝘢𝘳 𝘦𝘴𝘵𝘦 𝘤𝘰𝘮𝘢𝘯𝘥𝘰. 𝘕𝘦𝘤𝘦𝘴𝘪𝘵𝘰 𝘢 𝘭𝘰𝘴 𝘫𝘦𝘧𝘦𝘴 𝘢𝘲𝘶𝘪́',
  ...
  if (!msg) return
  // Extraer solo contextInfo del global.fake (quoted va separado)
  const _ctx = global.fake?.contextInfo || global.rcanal?.contextInfo || null
  return conn.sendMessage(m.chat, { text: msg, contextInfo: _ctx }, { quoted: m })
}
...
export async function handler(chatUpdate) {
  try {
    ...
    // Ejecutar plugins con handler.all (como _allfake)
    for (const name in global.plugins) {
      const plug = global.plugins[name]
      if (plug?.all && typeof plug.all === 'function') {
        try { await plug.all.call(conn, m) } catch (e) { console.log(`❌ all error ${name}:`, e) }
      }
    }
